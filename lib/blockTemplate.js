...
var BlockTemplate = module.exports = function BlockTemplate(
    ...
    this.rpcData.round = this.rpcData.round || 0;
    ...
    masternodePayments = rpcData.masternode_payments;
    ...
    this.serializeHeader = function(nTime, nonce){
        var isSapling = parseInt(this.rpcData.version) >= SAPLING_BLOCK_VERSION;
        var header =  new Buffer(140 + (isSapling ? 4 : 0));
        var position = 0;
        ...
        */
        ...
        if (isSapling) {
            header.writeUInt32LE(this.rpcData.round, position += 4, 4, 'hex');
        }
    ...
    this.serializeBlock = function(header, soln){
        ...
        // Empty vSig
        if (header.readInt32LE(0) >= SAPLING_BLOCK_VERSION) {
    ...
    this.getJobParams = function(){
        if (!this.jobParams){
            this.jobParams = [
                ...
                util.packUInt32LE(this.rpcData.round).toString('hex'),
                true
            ];
        }
